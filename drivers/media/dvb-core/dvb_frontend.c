			...
			unsigned int cmd, void *parg)
{
	...
	case FE_DISEQC_SEND_BURST:
		if (fe->ops.diseqc_send_burst) {
		        enum fe_sec_mini_cmd cmd = *(enum fe_sec_mini_cmd*)parg;
			err = fe->ops.diseqc_send_burst(fe, cmd);
			fepriv->state = FESTATE_DISEQC;
			fepriv->status = 0;
		}
		break;
	...
	case FE_SET_TONE:
		if (fe->ops.set_tone) {
			enum fe_sec_tone_mode tone = *(enum fe_sec_tone_mode*)parg;
			err = fe->ops.set_tone(fe, tone);
			fepriv->tone = tone;
			fepriv->state = FESTATE_DISEQC;
			fepriv->status = 0;
		}
	...
	case FE_SET_VOLTAGE:
		if (fe->ops.set_voltage) {
			enum fe_sec_voltage voltage = *(enum fe_sec_voltage*)parg;
			err = fe->ops.set_voltage(fe, voltage);
			fepriv->voltage = voltage;
			fepriv->state = FESTATE_DISEQC;
			fepriv->status = 0;
		}
		break;
